      ...
      <div className="py-[100px] bg-[#F5F8FA] px-[60px]">
        ...
        <div className="mt-[62px] grid grid-cols-4 gap-6">
        {
            workingProcess?.map((item)=>(
                <div className='flex flex-col items-center justify-center text-center' key={item.id}>
                    <Image src={item?.image} width={253} height={253} alt=''/>
                    <h2 className='text-[22px] font-semibold pt-[30px]'>{item?.name}</h2>
                    <p className='pt-2 text-base leading-7 w-[233px]'>{item?.title}</p>
                </div>
            ))
        }
